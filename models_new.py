...
class GaitDetectorSSL:
    ...
    def cross_val(self, X, Y, params, wandb_flag, groups=None, return_f1=False):
        # train walk detector & cross-val-predict
        if self.verbose:
            print("Running cross_val_predict...")

        Wlog, Wp, cv_test_idxs, Wlog_train, Wp_train, cv_train_idxs = cvp(
        ...
        self.fit(X, Y, params, wandb_flag, groups=groups)
    ...
    def fit(self, X, Y, wandb_flag, params, groups=None):
        ...
        train_dataset = sslmodel.NormalDataset(x_train, y_train, pid=group_train, name="training",augmentation=augmentation)
        ...
        if wandb_flag:
            wandb.log({"augmentation":  np.mean(augmentation)})
        # balancing to 90% notwalk, 10% walk
        
        class_weights = self._get_weight_class(y_train)

        model = self._get_model(pretrained=True)
    ...
    def _get_model(self, pretrained):
        return sslmodel.get_sslnet(tag=self.repo_tag, pretrained=pretrained)
    
    def _get_weight_class(self, y_train):
        walk = np.sum(y_train[:,1])
        notwalk = np.sum(y_train[:, 0])
        return [(walk * 9.0) / notwalk, 1.0]
...
class GaitChoreaDetectorSSL(GaitDetectorSSL):
    def __init__(
            self,
            window_sec=6,
            sample_rate=30,
    ...
    def _get_model(self, pretrained):
        return sslmodel.get_sslnet(tag=self.repo_tag, pretrained=pretrained, num_classes=self.num_classes)
...
def cvp(
    ...
    def worker(train_idxs, test_idxs):
        ...
        if fit_predict_groups:
            ...
            Y_train_logits,Y_train_pred = getattr(m, method)(X_train, params, groups=groups_train)
        else:
            m.fit(X_train, Y_train)
            Y_test_logits,Y_test_pred = getattr(m, method)(X_test, params)
            Y_train_logits,Y_train_pred = getattr(m, method)(X_train, params, groups=groups_train)

        return Y_test_logits, Y_test_pred, test_idxs, Y_train_logits,Y_train_pred, train_idxs 
    ...
    Y_logits_train = np.concatenate([r[3] for r in results])
    Y_pred_train = np.concatenate([r[4] for r in results])
    cv_test_idxs = [r[2] for r in results]
    cv_train_idxs = [r[5] for r in results]

    if return_indices:
        return Y_logits, Y_pred, cv_test_idxs, Y_logits_train, Y_pred_train, cv_train_idxs
